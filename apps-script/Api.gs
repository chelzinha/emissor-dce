...
function doPost(event) {
  try {
    ...
    const handlers = {
      ...
      'campaign.company.get': function() { return campaignCompanyGet_(user.id, request.payload || {}); },
      'campaign.company.upsert': function() { return campaignCompanyUpsert_(user.id, request.payload || {}); },
      ...
      'postalObjects.list': function() { return listPostalObjects_(user.id, request.payload || {}); },
      'production.prepare': function() { return prepareProductionBatch_(user.id, request.payload || {}); },
      'production.list': function() { return listProductionBatches_(user.id, request.payload || {}); },
      'production.dce.prepare': function() { return prepareProductionDce_(user.id, request.payload || {}); },
      'production.dce.list': function() { return listProductionDceForClient_(user.id, request.payload || {}); },
      'production.dce.reserve': function() { return reserveProductionDce_(user.id, request.payload || {}); },
      'production.dce.saveResults': function() { return saveProductionDceResults_(user.id, request.payload || {}); },
